...
class ProductController extends AppBaseController
{
    ...
    public function create()
    {
        ...
        return view('shop.products.create')
                ...
                ->with('model_required',\Zcjy::modelRequiredParam($this->productRepository->model()));
    }
    ...
    public function store(CreateProductRequest $request)
    {
        ...
        #存储商品会员等级价格
        app('commonRepo')->productLevelPriceRepo()->saveLevelPrice($input,$product->id);
    ...
    public function edit(Request $request,$id)
    {
     ...
     //  return $wordlist;
       //return $selectedWords;
        ...
        //模型必须项
        $model_required = \Zcjy::modelRequiredParam($this->productRepository->model());
        $spec_show = 1;
        return view('shop.products.edit', compact('wordlist','selectedWords','spec_show','edit_rember','images', 'product', 'categories', 'brands', 'types','level','level01', 'level02','level03','fist_cats','second_cats','third_cats', 'countries','stores','selectedStores','model_required'));
    }
    ...
    public function update($id, Request $request)
    {
        ...
        #存储商品会员等级价格
        app('commonRepo')->productLevelPriceRepo()->saveLevelPrice($input,$product->id,'update');
    ...
    public function wordsList(){
        $wordList=Word::all();
        //return $wordList;
        return view('shop.products.wordlist')
                ->with('wordList',$wordList);
    }

    public function wordsListAdd(){
        return view('shop.products.wordlist_add');
    }
    ...
    public function ajaxGetSpecSelect(Request $request)
    {
        ...
        */
        ...
        return view('shop.products.ajax_spec', compact('items_ids', 'specList')); 
    }
    ...
    public function ajaxGetSpecInput(Request $request, $product_id)
    {
        ...
        */
        ...
        foreach ($keySpecGoodsPrice_tmp as $key => $value) {
            ...
            $keySpecGoodsPrice[$value->key] = app('commonRepo')->productLevelPriceRepo()->addArrSpecLevelPrice(['key_name' => $value->key_name, 'inventory' => $value->inventory,'price' => $value->price,'sku' => $value->sku,'image'=>$value->image,'jifen'=>$value->jifen,'id'=>$value->id],$value->id);
        }
        ...
        #附带单元格信息上去
        $str  = app('commonRepo')->productLevelPriceRepo()->addSpecTr($str); 
        ...
        foreach ($spec_arr2 as $k => $v) 
        {
           ...
           // $i++;
            ...
            // if (array_key_exists($item_key, $keySpecGoodsPrice) && $this->varifyArrKey($keySpecGoodsPrice[$item_key])) {

            // $level_price_arr = $this->getLikeKeyArr($keySpecGoodsPrice[$item_key]);
            ...
            #附带input信息
            $str = app('commonRepo')->productLevelPriceRepo()->addSpecInput($str,$item_key,isset($keySpecGoodsPrice[$item_key]) ? $keySpecGoodsPrice[$item_key]['id'] : 0);
    ...
    private function varifyArrKey($arr,$vari_key='level_price_')
    {
        $status = 1;
        if(!is_null($vari_key))
        {
    ...
    private function getLikeKeyArr($arr,$like_key = 'level_price_')
    {
        $like_arr = [];
        if(count($arr))
        {
        ...
        if(array_key_exists('item',$input)) {
            ...
            foreach ($input['item'] as $key => $item) 
            {
                if (!is_null($item['price'])) 
                {
                    ...
                    //再存入规格信息
                    app('commonRepo')->productLevelPriceRepo()->saveLevelPrice($item,$spec->id,'create','spec');
    ...
    public function allLowGoods(Request $request){
        ...
        return view('shop.products.all_low'.$type)
            ->with('tools',$tools)
            ->with('cats',$cats)
            ->with('second_cats', $second_cats)
            ->with('third_cats',$third_cats)
    ...
    public function searchGoodsFrame(Request $request){
        ...
        $product_num=$i;
        return view('shop.products.search_tem'.$type)
