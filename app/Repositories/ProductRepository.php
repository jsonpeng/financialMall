...
class ProductRepository extends BaseRepository
{
     ...
     */
    public function creditProducts($skip = 0,$take = 20)
    {
      return Cache::remember('products_'.$skip.'_'.$take, Config::get('web.cachetime'), function() use($skip, $take){
            $products = Product::where('shelf', 1)
                 ->where('price',0)
                 ->where('jifen','>',0)
                 ->select('id', 'name', 'jifen' ,'image', 'inventory', 'max_buy', 'weight', 'remark', 'sort', 'shelf', 'is_new', 'is_hot', 'views', 'sales_count')
                ->orderBy('sort', 'desc')
                ->orderBy('is_hot', 'desc')
     ...
     * @return [type]          [description]
     */
    public function varifyProductType($product)
    {
      $product['type'] = '普通商品';
      if($product->price == 0 && $product->jifen > 0)
      {
        $product['type'] = '积分商品';
      }
      return $product;
     ...
     * @return [type]           [description]
     */
    public function attachMemberPrice($products)
    {
        foreach ($products as $key => $product) 
        {
            $product['member_price'] = app('commonRepo')->productLevelPriceRepo()->getAllLevelWithPrice($product->id);
        }
        return $products;
    }
    ...
    public function getRecommendProducts($skip = 0, $take = 20){
        return Cache::remember('newProducts_'.$skip.'_'.$take, Config::get('web.cachetime'), function() use($skip, $take){
            ...
            #附带会员价格
            $products = $this->attachMemberPrice($products);
        ...
        return Cache::remember('relatedProducts'.$id.'_'.$take, Config::get('web.cachetime'), function() use($id, $take){
            ...
            foreach ($products as $key => $value) 
            {
                if ($value->prom_type) {
                 ...
                 $value['user_price'] = $value['realPrice'];
            }
            #附带会员价格
            $products = $this->attachMemberPrice($products);
            return $products;
     ...
     * @return [type]         [description]
     */
    public function getProductsCached($cat_id, $skip = 0, $take = 18,$user = null){
        return Cache::remember('get_products_of_cat_'.$cat_id.'_'.$skip.'_'.$take.$user, Config::get('web.cachetime'), function() use($cat_id, $skip, $take,$user){
            ...
            foreach ($products as $key => $value) {
                ...
                $value['user_price'] = $value['realPrice'];
                //app('commonRepo')->getMemberPrice($user,$value);
                $value = $this->varifyProductType($value);
            }
            #附带会员价格
            $products = $this->attachMemberPrice($products);
            return $products;
    ...
    //获取某分类，及其子分类下的产品信息
    public function getProductsOfCatWithChildrenCatsCached($cat_id, $skip = 0, $take = 18,$user=null){
        return Cache::remember('getProductsOfCatWithChildrenCatsCached'.$cat_id.'_skip'.$skip.'_take'.$take.$user, Config::get('web.cachetime'), function() use($cat_id, $skip, $take,$user){
                #获取子分类集合
    ...
    public function deleteAttachInfoByProduct($product){
        ...
        //加入到购物车的商品
        \App\Models\Item::whereNull('order_id')->where('product_id',$product->id)->delete();
        //WORD
        $product->words()->sync([]);
        //促销信息
        #商品促销
