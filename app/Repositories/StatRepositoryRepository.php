...
class StatRepositoryRepository extends BaseRepository
{
    ...
    public function successOrdersId()
    {
        $orders = Order::where([
                    ['order_status', '<>', '无效'],
                    ['order_status', '<>', '已取消'],
    ...
    {
        $ordersId = $this->successOrdersId();
        return Item::whereIn('order_id',$ordersId)->whereBetween('created_at', array($time_begin, $time_end))->select('name', 'unit', DB::raw('SUM(count) as total_sales'), DB::raw('SUM(price) as total_prices'))->groupBy('name', 'unit')->get();
    ...
    public function rangeSaleStats($time_begin, $time_end)
    {
        return Order::where([
            ...
            ->whereBetween('created_at', array($time_begin, $time_end))
