...
<div class="content pdall0-xs" style="max-width: 990px;">
    <div class="box box-solid">
        ...
        <div class="box-body" style="padding: 10px;">
            ...
            <a class="btn pull-right btn-primary" href="/shop/order/print/{{ $order->id }}">订单打印</a>
        </div>
        <!-- /.box-body --> 
    </div>
        ...
        <div class="box-body">
            ...
            <div class="row" style="border-bottom: 1px solid #f4f4f4" >
                <form id="item_form_{{ $item->
                    id }}">
                    <a class="col-md-1 col-xs-2 hidden-xs" style="line-height: 28px;" href="/shop/products/{{ optional($item->product)->id }}/edit" target="_blank">{{ $item->id }}</a>
                    ...
                    <div class="col-md-1 col-xs-1" id="item_jifen_{{ $item->
                        id }}">
  ...
  @if(isset($order->invoice) && isset($order->invoice_type) && !empty($order->invoice_type))
  <div class="box box-solid">
        <div class="box-header with-border">
        ...
        <div class="box-body">
            <div class="row">
                <form id="price_adjust">
                    <div class="col-md-3"><label>发票类型:</label> {!! $order->invoice_type !!}</div>
                    <div class="col-md-3"><label>发票抬头:</label> {!! $order->invoice_title !!}</div>
                    <div class="col-md-3"><label>税号:</label> {!! $order->tax_no !!}</div>
                </form>
            </div>
        </div>
        <!-- /.box-body --> 
...
@include('shop.orders.partial.js')
<script type="text/javascript">
    ...
    function orderAddProductApi(spec_id,product_id){
       ...
       $.ajax({
        url:"/shop/orders/addProductList" ,
        type:"POST",
    ...
    function orderDelProductApi(item_id,order_id,product_id){
       ...
       $.ajax({
        url:"/shop/orders/delProductList/"+item_id,
        type:"POST",
        success:function(data){
            if(data.code==0){
               console.log("删除成功");
    ...
    function addProductMenu(){
            ...
            $.ajax({
            url:"/shop/products/ajaxGetProductList" ,
            type:"POST",
            success:function(data){
            if(data.code==0){
                ...
                $('.productlist').each(function(){
                    var product_id=$(this).data('productid');
                       ...
                       $.ajax({
                        url:"/shop/products/getSingleProductById/"+product_id,
                        type:"POST",
                        success:function(data){
                            if(data.code==0){
