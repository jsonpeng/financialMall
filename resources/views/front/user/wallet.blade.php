...
@section('content')
    <div class="user-header">
        <div class="header" style="background-color: transparent;">
            <a href="javascript:history.go(-1)" class="go_return">返回</a><p>我的钱包</p>
        </div>
    ...
    <div style="margin: 0 15px;margin-top: -45px;background-color: #fff;border-radius: 5px; margin-bottom: 55px;">
        <div class="inner-list">
            <div class="sec-wrapper">
                <a class="sec" href="/wallet"><div class="title active">提现记录</div></a>
                <a class="sec" href="/income?type=1"><div class="title">贷款收入</div></a>
                <a class="sec" href="/income?type=2"><div class="title">推广收入</div></a>
            </div>
            <div style="padding: 10px;" class="scroll-container">
                @foreach ($cashList as $element)
                    <div class="weui-media-box weui-media-box_text scroll-post">
                        <h4 class="weui-media-box__title">¥{{ $element->count }} <span>{{ $element->status }}</span></h4>
                        <ul class="weui-media-box__info">
                            <li class="weui-media-box__info__meta">申请时间：{{ $element->created_at }}</li>
                        </ul>
                        @if ($element->status == '失败')
                            <ul class="weui-media-box__info">
                                <li class="weui-media-box__info__meta">{{ $element->reason }}</li>
                            </ul>
                        @endif
                    </div>
                @endforeach
